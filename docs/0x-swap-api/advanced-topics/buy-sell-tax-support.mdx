...
![Generic Swap Error](/docs/static/img/0x-swap-api/generic-swap-error.png)
...
<Tip>
⚡️ Explore the Swap API v2 [demo code repo](https://github.com/0xProject/0x-examples/blob/main/swap-v2-allowance-holder-next-app/app/components/price.tsx#L362-L375) to see how to display the buy/sell tax for a selected token.
...
Use the `tokenMetadata` object returned in the `/price` and `/quote` endpoints for both [Swap and Gasless API calls](/api-reference/swap/getprice-allowance-holder). This object contains the buy/sell tax information for the tokens involved in the swap.
