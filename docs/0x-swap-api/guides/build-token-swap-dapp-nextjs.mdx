...
<Tip>
  ⚡️ Quicklink: Swap API v2 AllowanceHolder [demo
...
We’ll build our token swap app on [Base](https://www.base.org/), but you can adapt it to any [supported chain](/docs/introduction/supported-chains).
...
![Swap v2 dApp Core Concepts](/docs/static/docs/static/img/0x-swap-api/swap-v2-core-concepts-allowance-holder.png)
...
4. **Approve token allowance**
   ...
   - If the user has enough of the sell token, the button changes to **“Approve”**.
   - Approving grants the required allowance to AllowanceHolder so the app can transfer ERC-20 tokens on the user’s behalf.
   - Users can set either the default (large) allowance or a custom amount equal to their balance.

5. **Review and confirm**
   - Once the allowance is approved, the button changes to **“Review Trade”**.
