...
- increased security via the [AllowanceHolder contract](/docs/core-concepts/contracts#allowanceholder-recommended) or [Permit2 contract](/docs/core-concepts/contracts#permit2-advanced-use-only) for allowances
...
- new [buy/sell tax support](/docs/0x-swap-api/advanced-topics/buy-sell-tax-support)
...
Find the [latest API reference here](/api-reference/swap).
...
In v2, for [tokens that do not support gasless approvals](/api-reference/gasless/getgaslessapprovaltokens), use the [standard token approval flow](/docs/0x-swap-api/advanced-topics/how-to-set-your-token-allowances).

<Danger>
When using the [Standard Approval Flow](/docs/0x-swap-api/advanced-topics/how-to-set-your-token-allowances) (aka Gasless Approvals are not supported)
